...
class AdvancedChessBot:
    ...
    def select_move(self):
        ...
        try:
            # Log current board state if debug is enabled
            if self.debug:
                self.debug_board_state()
            ...
            if self.board.fullmove_number <= 2:
                ...
                if self.debug:
                    print(f"Using random opening move: {random_move.uci()}")
                return random_move
            ...
            if best_move is None:
                if self.debug:
                    print("Minimax returned None move. Falling back to random move.")
                return random.choice(legal_moves)
            
            if best_move not in legal_moves:
                if self.debug:
                    print(f"Minimax returned illegal move: {best_move.uci()}. Falling back to random move.")
                return random.choice(legal_moves)
            
            if self.debug:
                print(f"Selected move: {best_move.uci()}")
            ...
            if self.debug:
                print(f"Error in select_move: {str(e)}")
    ...
    def make_move(self, move_uci):
        ...
        if self.debug:
            print(f"Attempting to make move: {move_uci}")
        ...
        try:
            ...
            if move in self.board.legal_moves:
                ...
                if self.debug:
                    print(f"Move {move_uci} successfully applied")
                    self.debug_board_state()
                return True
            else:
                if self.debug:
                    print(f"Move {move_uci} is not legal in current position")
                    self.debug_board_state()
                return False
        except ValueError as e:
            if self.debug:
                print(f"Invalid UCI format: {move_uci}, error: {str(e)}")
            return False
        except Exception as e:
            if self.debug:
                print(f"Unexpected error making move {move_uci}: {str(e)}")
                print(traceback.format_exc())
            return False
    
    def debug_board_state(self):
        """Print debug information about the current board state."""
        print(f"Current board FEN: {self.board.fen()}")
        ...
        print(f"Move count: {self.board.fullmove_number}")
        
        # Check for any special conditions
        if self.board.is_check():
            print("Board is in check")
        if self.board.is_checkmate():
            print("Board is in checkmate")
        if self.board.is_stalemate():
            print("Board is in stalemate")
