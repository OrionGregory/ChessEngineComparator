...
if __name__ == "__main__":
    stockfish_path = "C:\\Users\\catch\\Desktop\\stockfish\\stockfish-windows-x86-64-avx2.exe"  # Or use a raw string: r"C:\..."
    print(f"Stockfish path: {stockfish_path}")  # Crucial: Print the path!

    if os.path.exists(stockfish_path):  # Check if the file exists
        print("Stockfish executable found!")
        bot = SimpleChessBot(stockfish_path=stockfish_path)
        bot.play_game()
    else:
        print(f"Error: Stockfish executable not found at {stockfish_path}")  # Handle the error